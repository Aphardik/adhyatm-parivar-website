...
const Page = () => {
  ...
  const data = content?.sanskritShikshan;

  const statsData = data?.stats?.map((stat) => ({
    count: stat.count,
    label: stat.label,
    color: stat.count === "225" ? "bg-gradient-to-b from-yellow-400 to-orange-500" :
      stat.count === "600" ? "bg-gradient-to-b from-green-400 to-green-600" :
        ...
        <section className="pt-8 sm:pt-12 lg:pt-16">
          <div className="grid sm:grid-cols-1 items-center gap-6 sm:gap-10">
            <div className="relative rounded-sm p-4 sm:p-12 bg-lightyellow">
              ...
              <p className='font-semibold text-darkpink text-xl'>{data?.intro?.highlight}</p>
            </div>
