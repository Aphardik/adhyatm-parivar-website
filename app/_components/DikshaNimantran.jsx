...
const HighlightedTitle = ({ title }) => {
    ...
    if (match) {
        const quote = match[1];
        const restOfText = title.replace(`'${quote}'`, '').trim();
        return (
            <div className="flex flex-col gap-1">
                <span className="text-maroon font-black text-xl md:text-3xl italic block mb-1">
                    '{quote}'
                </span>
                {restOfText && (
                    <span className="text-gray-800 text-lg md:text-2xl font-bold leading-relaxed">
                        {restOfText}
                    </span>
                )}
            </div>
    ...
    return (
        <p className="text-gray-800 text-lg md:text-2xl font-bold leading-relaxed">
            {title}
        </p>
    );
};
                                    ...
                                    <React.Fragment key={eIdx}>
                                        <div className="flex flex-col items-center text-center gap-2 group/item">
                                            ...
                                            <div className="flex-1">
                                                <HighlightedTitle title={event.title} />
                                            </div>
                                        </div>
                                        {eIdx < day.events.length - 1 && (
                                            <div className="flex justify-center">
                                                <div className="w-full max-w-[80%] h-px bg-gradient-to-r from-transparent via-maroon/10 to-transparent"></div>
                                            </div>
                                        )}
                                    </React.Fragment>
                                ))}
