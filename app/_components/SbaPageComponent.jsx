...
const PhotoGallery = () => {
  ...
  // Visible photos based on lazy loading
  ...
  const loadMoreImages = useCallback(() => {
    if (visibleCount >= sortedPhotos.length || isLoadingMore) return;
    
    setIsLoadingMore(true);
    setTimeout(() => {
      setVisibleCount(prev => Math.min(prev + 10, sortedPhotos.length));
      setIsLoadingMore(false);
    }, 300);
  }, [visibleCount, sortedPhotos.length, isLoadingMore]);
  ...
  // Reset lazy loading when language changes
  useEffect(() => {
    setVisibleCount(10);
  ...
  return (
    ...
    >
      ...
      {/* Photo Grid */}
      <main className="max-w-7xl mx-auto  py-2 sm:py-4">
              ...
              <span className="text-sm">
                {isLoadingMore 
                  ? (language === "hindi" 
                      ...
                      : "Loading more...")
                  : (language === "hindi" 
                      ? "नीचे स्क्रॉल करें और अधिक लोड करें" 
                      : language === "gujarati" 
                      ? "નીચે સ્ક્રોલ કરો અને વધુ લોડ કરો" 
        ...
        {/* End of Gallery */}
        {visibleCount >= sortedPhotos.length && sortedPhotos.length > 0 && (
          <div className="text-center py-8 text-gray-500 text-sm">
            {language === "hindi" 
              ? "सभी फोटो लोड हो गई हैं" 
              : language === "gujarati" 
              ? "બધી ફોટો લોડ થઈ ગઈ છે" 
              : "All photos loaded"
            }
          </div>
        )}
